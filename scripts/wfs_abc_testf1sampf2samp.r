...
# assumes f1samp is not normalized in thisout.ff
runabc <- function(locusnum, thistarg, thisout.ff, tol, f1samp.mean, f1samp.sd){

	# with f1samp as a summary stat
	# calc euclidean distance
	sum1 <- ((thisout.ff['f1samp',]-f1samp.mean)/f1samp.sd - thistarg[,f1samp.n])^2
...
runabcf1f2 <- function(locusnum, thistarg, thisout.ff, tol){
	...
	# set up return matrix
	ret <- matrix(data=c(rep(locusnum, sum(wt1)), thisout.ff['f1samp',wt1], thisout.ff['f2samp',wt1], thisout.ff['fsdprime',wt1], thisout.ff['fsiprime',wt1], thisout.ff['ne',wt1], thisout.ff['f1',wt1], thisout.ff['s',wt1]), ncol=8, byrow=FALSE)
...
#locnum <- 37461; sampsize <- '38,40'
locnum <- 7153; sampsize <- '38,48'
...
summary(thisout.ff['f1samp',]) # test whether f1samp was normalized

i <- targ[locnum,] # the input object
	
results.fsi <- runabc(locusnum=i[,locusnum], thistarg=i[,.(f1samp.n, fsd.n, fsi.n)], thisout.ff=thisout.ff, tol=tol, f1samp.mean=meansds$f1samp.mean[meanind], f1samp.sd=meansds$f1samp.sd[meanind]) # run abc analysis with f1samp and fsi/fsd (the old way)

results.f1f2 <- runabcf1f2(locusnum=i[,locusnum], thistarg=i[,.(f1samp, f2samp)], thisout.ff=thisout.ff, tol=tol) # run abc with unscaled f1samp and f2samp (the possible new way)
