...
# the abc simulations
ffnm <- paste('analysis/temp/wfs_sims_ff', '44,44', sep='')
...
### basic plots
i <- 1:2000 # selection a few simulations to simplify plotting
...
pdf('analysis/figures/wfs_abc_sims_s_vs_diff_by_f1samp.pdf', width=9, height=4)
par(mfrow=c(2,5))
...
for(j in 1:length(ints)){
	k <- which(thisout.ff['f1samp',i] > ints[[j]][1] & thisout.ff['f1samp',i] < ints[[j]][2])
