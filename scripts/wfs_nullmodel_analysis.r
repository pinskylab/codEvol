...
suffix <- '_07-14_25k'
locnms11 <- fread('data/data_29.06.17/Frequency_table_Lof07_Lof11_25k.txt', header=TRUE); setnames(locnms11, 3:7, c('N_CHR_1', 'Freq_1', 'N_CHR_2', 'Freq_2', 'ABS_DIFF')) # for 1907-2011 (for comparison)
...
##################
# most diverged loci?
sum(selinds <- dat$p.adj <0.1 & !(dat$CHROM %in% c('LG01', 'LG02', 'LG07', 'LG12', 'Unplaced'))) # not on Unplaced (the ones we want)
sum(dat$p.adj <0.1 & dat$CHROM %in% c('LG01', 'LG02', 'LG07', 'LG12')) # within inversions
sum(dat$p.adj <0.1 & dat$CHROM %in% c('Unplaced')) # on Unplaced
...
#	plot(dat$ABS_DIFF, -log10(dat$p.adj),type='p', cex=0.2, xlab='Frequency change', ylab='-log10(FDR-adjusted p)', col=rgb(0.1,0.1,0.1, 0.2))
	plot(dat$ABS_DIFF, -log10(dat$p.adj),type='p', cex=0.2, xlab='Frequency change', ylab='-log10(FDR-adjusted p)', col=colrmpfun(dat[,cntmn/49]))
