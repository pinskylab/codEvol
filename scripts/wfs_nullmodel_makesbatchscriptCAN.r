...
outfile <- 'scripts/wfs_nullmodel_submit_all_sbatchCAN.sh'

cat('#!/bin/bash\n', file=outfile, append=FALSE) # header

for(i in which(nloci[,todo==1])){
	cat(paste('sbatch --job-name=nlCN', nloci[i,alcnt1], ',', nloci[i,alcnt2], ' --cpus-per-task=', min(16, nloci[i,nloci]), ' scripts/wfs_nullmodel_sbatchCAN.sh ', nloci[i,alcnt1], ' ', nloci[i, alcnt2], ' ', min(16, nloci[i,nloci]), '\n', sep=''), file=outfile, append=TRUE)
}