...
# set up 1000 not-outlier loci
	...
	# for Canada
	outl[outlierCan_q3==1, nearoutlierCAN:=TRUE] # list of loci that are outliers
	for(i in outl[,which(outlierCan_q3==1)]) { # add loci that are <= 5000 bp from outliers
		outl[CHROM==outl$CHROM[i] & abs(POS - outl$POS[i]) <= 5000, nearoutlierCAN:=TRUE]
	}
	allloci <- outl[nearoutlierCAN==FALSE & !is.na(Freq_Can40) & !is.na(Freq_CanMod), .(CHROM, POS)] # list of all loci that aren't near outliers
	allloci$dist = NA
	allloci$dist[2:nrow(allloci)] <- allloci$POS[2:nrow(allloci)] - allloci$POS[1:(nrow(allloci)-1)] # calculate distance to next locus to the left
		dim(allloci)
	allloci <- allloci[dist > 5000 & POS > 5000,] # trim out loci <5000 bp from start of CHROM or from another locus (doesn't check end of CHROM...)
		dim(allloci)
	notoutl <- allloci[sample(.N, 1000)] # sample 1000 loci to be the "not outliers"
	notoutl[,notoutlierCAN:=TRUE] # label these as the 1000 not outliers
	notoutl[,dist:=NULL] # remove dist column
	outl <- merge(outl, notoutl, by=c('CHROM', 'POS'), all.x=TRUE) # label the not-outliers
...
# calculate initial frequency of the allele that increases in frequency
...
legend('topright', legend=c('LOF_S_14', 'LOF_S_11', 'CANMod', 'Outlier', 'Not outlier'), col=c(cols[c(1,3,2)], 'black', 'black'), lty=c(rep(1,3), 3), bty='n', cex=0.5)
