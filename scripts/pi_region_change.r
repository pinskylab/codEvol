...
dat14 <- fread(paste('analysis/LOF_S_14_', windnm, '.windowed.pi', sep=''))
dat11 <- fread(paste('analysis/LOF_S_11_', windnm, '.windowed.pi', sep=''))
dat07 <- fread(paste('analysis/LOF_07_', windnm, '.windowed.pi', sep=''))
dat40 <- fread(paste('analysis/CAN40_', windnm, '.windowed.pi', sep=''))
datMod <- fread(paste('analysis/CANMod_', windnm, '.windowed.pi', sep=''))
...
bins[,pi_region_perc0711 := ecdf0711(pi_diff_0711)]
bins[,pi_region_perc0714 := ecdf0714(pi_diff_0714)]
bins[,pi_region_percCAN := ecdfCan(pi_diff_CAN)]

	# examine
	bins[pi_region_perc0711>0.995 & pi_region_perc0714>0.995 & pi_region_percCAN>0.995,]
	bins[pi_region_perc0711<0.005 & pi_region_perc0714<0.005 & pi_region_percCAN<0.005,]
...
# group the 1% outliers (increasing or decreasing)
	...
	for(i in 1:length(chroms)){
		...
		temp0711 <- c(NA, bins[CHROM==chroms[i] & pi_region_perc0711>0.995, BIN_START])
		temp0714 <- c(NA, bins[CHROM==chroms[i] & pi_region_perc0714>0.995, BIN_START])
		tempCAN <- c(NA, bins[CHROM==chroms[i] & pi_region_percCAN>0.995, BIN_START])
		...
		bins[CHROM==chroms[i] & pi_region_perc0711>0.995, dist0711:= as.numeric(BIN_START - temp0711)]
		bins[CHROM==chroms[i] & pi_region_perc0714>0.995, dist0714:= as.numeric(BIN_START - temp0714)]
		bins[CHROM==chroms[i] & pi_region_percCAN>0.995, distCAN:= as.numeric(BIN_START - tempCAN)]
	} 

	# label increasing D clusters (a little slow)
	indx <- 1
	bins[,pi_region_cluster0711 := as.numeric(NA)]
	outls <- which(bins$pi_region_perc0711>0.995)
	for(i in 1:length(outls)){
		bins$pi_region_cluster0711[outls[i]] <- indx
		if(i < length(outls)){
	...
	indx <- 1
	bins[,pi_region_cluster0714 := as.numeric(NA)]
	outls <- which(bins$pi_region_perc0714>0.995)
	for(i in 1:length(outls)){
		bins$pi_region_cluster0714[outls[i]] <- indx
		if(i < length(outls)){
	...
	indx <- 1
	bins[,pi_region_clusterCAN := as.numeric(NA)]
	outls <- which(bins$pi_region_percCAN>0.995)
	for(i in 1:length(outls)){
		bins$pi_region_clusterCAN[outls[i]] <- indx
		if(i < length(outls)){
	...
	for(i in 1:length(chroms)){
		# offset the locations by one outlier locus
		temp0711 <- c(NA, bins[CHROM==chroms[i] & pi_region_perc0711<0.005, BIN_START])
		temp0714 <- c(NA, bins[CHROM==chroms[i] & pi_region_perc0714<0.005, BIN_START])
		tempCAN <- c(NA, bins[CHROM==chroms[i] & pi_region_percCAN<0.005, BIN_START])
		...
		# subtract to get the distance
		bins[CHROM==chroms[i] & pi_region_perc0711<0.005, dist0711dec:= as.numeric(BIN_START - temp0711)]
		bins[CHROM==chroms[i] & pi_region_perc0714<0.005, dist0714dec:= as.numeric(BIN_START - temp0714)]
		bins[CHROM==chroms[i] & pi_region_percCAN<0.005, distCANdec:= as.numeric(BIN_START - tempCAN)]
	} 

	# label increasing D clusters (a little slow)
	indx <- 1
	bins[,pi_region_cluster0711dec := as.numeric(NA)]
	outls <- which(bins$pi_region_perc0711<0.005)
	for(i in 1:length(outls)){
		bins$pi_region_cluster0711dec[outls[i]] <- indx
		if(i < length(outls)){
	...
	indx <- 1
	bins[,pi_region_cluster0714dec := as.numeric(NA)]
	outls <- which(bins$pi_region_perc0714<0.005)
	for(i in 1:length(outls)){
		bins$pi_region_cluster0714dec[outls[i]] <- indx
		if(i < length(outls)){
	...
	indx <- 1
	bins[,pi_region_clusterCANdec := as.numeric(NA)]
	outls <- which(bins$pi_region_percCAN<0.005)
	for(i in 1:length(outls)){
		bins$pi_region_clusterCANdec[outls[i]] <- indx
		if(i < length(outls)){
	...
	# examine
	bins[,range(table(pi_region_cluster0711))] # all clusters only 1 window wide
	bins[,range(table(pi_region_cluster0714))] # all clusters only 1 window wide
	bins[,range(table(pi_region_clusterCAN))] # all clusters only 1 window wide
	bins[,range(table(pi_region_cluster0711dec))] # all clusters only 1 window wide
	bins[,range(table(pi_region_cluster0714dec))] # all clusters only 1 window wide
	bins[,range(table(pi_region_clusterCANdec))] # all clusters only 1 window wide
...
bins[(pi_region_perc0711<=0.995 | pi_region_perc0714<=0.995 | pi_region_percCAN<=0.995) & (pi_region_perc0711>=0.005 | pi_region_perc0714>=0.005 | pi_region_percCAN>=0.005), plot(POSgen, pi_diff_0711, type='p', cex=0.2, lwd=0.3, xaxt='n', xlab='', ylab='Pi change Lof0711', col=lgcol, las=1)]
bins[pi_region_perc0711>0.995 & pi_region_perc0714>0.995 & pi_region_percCAN>0.995, points(POSgen, pi_diff_0711, cex=0.5, col=cols[2])]
bins[pi_region_perc0711<0.005 & pi_region_perc0714<0.005 & pi_region_percCAN<0.005, points(POSgen, pi_diff_0711, cex=0.5, col=cols[3])]
...
bins[(pi_region_perc0711<=0.995 | pi_region_perc0714<=0.995 | pi_region_percCAN<=0.995) & (pi_region_perc0711>=0.005 | pi_region_perc0714>=0.005 | pi_region_percCAN>=0.005), plot(POSgen, pi_diff_0714, type='p', cex=0.2, lwd=0.3, xaxt='n', xlab='', ylab='Pi change Lof0714', col=lgcol, las=1)]
bins[pi_region_perc0711>0.995 & pi_region_perc0714>0.995 & pi_region_percCAN>0.995,points(POSgen, pi_diff_0714, cex=0.5, col=cols[2])]
bins[pi_region_perc0711<0.005 & pi_region_perc0714<0.005 & pi_region_percCAN<0.005, points(POSgen, pi_diff_0714, cex=0.5, col=cols[3])]

axis(side=1, at=chrs$pos, labels=gsub('LG','', chrs$CHROM), tcl=-0.3, cex.axis=0.7)

bins[pi_region_perc0711<=0.99 | pi_region_perc0714<=0.99 | pi_region_percCAN<=0.99, plot(POSgen, pi_diff_CAN, type='p', cex=0.2, lwd=0.3, xaxt='n', xlab='', ylab='Pi change CAN', col=lgcol, las=1)]
bins[pi_region_perc0711>0.995 & pi_region_perc0714>0.995 & pi_region_percCAN>0.995, points(POSgen, pi_diff_CAN, cex=0.5, col=cols[2])]
bins[pi_region_perc0711<0.005 & pi_region_perc0714<0.005 & pi_region_percCAN<0.005, points(POSgen, pi_diff_CAN, cex=0.5, col=cols[3])]
