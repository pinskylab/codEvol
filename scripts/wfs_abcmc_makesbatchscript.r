...
for(i in 1:nrow(nloci)){
	...
	exist1 <- file.exists(paste('analysis/temp/wfs_sims_ff', myalcnt1, ',', myalcnt2, '.RData', sep='')) # alternative model sims
	exist2 <- file.exists(paste('analysis/temp/wfs_simsnull_ff', myalcnt1, ',', myalcnt2, '.RData', sep='')) # null model sims
...
for(i in which(nloci[,todo==1])){
	cat(paste('sbatch --job-name=abc', nloci[i,alcnt1], ',', nloci[i,alcnt2], ' --cpus-per-task=', min(16, nloci[i,nloci]), ' scripts/wfs_abcmc_sbatch.sh ', nloci[i,alcnt1], ' ', nloci[i, alcnt2], ' ', min(16, nloci[i,nloci]), '\n', sep=''), file=outfile, append=TRUE)
}