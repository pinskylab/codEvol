...
# calculate p-value
outl[, p := pchisq(q = stat, df = 1, lower.tail = FALSE)]

# FDR correct by pop and type
outl[, pfdr := p.adjust(p, method = 'fdr'), by = .(ne, s, f, i)]
...
sum1 <- outl[, .(minp = min(p), minpfdr = min(pfdr), npfdrlow = sum(pfdr < 0.05)),  # min p and pfdr
              by = .(ne, s, f, i)]

sum2 <- sum1[, .(prop = sum(minpfdr <= 0.05)/.N), by = .(ne, s, f)]
############
# plot
############

# quick plot of each sim
p <- ggplot(outl[-log10(pfdr) > -log10(0.3),], aes(x=POS, y= -log10(pfdr))) + # only the low p-values
  geom_point(data = outl[-log10(pfdr) >= -log10(0.05),], size = 0.2, shape = 1, alpha = 0.3, color = 'red') +
  geom_point(data = outl[-log10(pfdr) > -log10(0.3) & -log10(pfdr) < -log10(0.05),], size = 0.2, shape = 1, alpha = 0.3, color = 'black') +
  facet_grid(ne + i ~ s + f, scales = 'free') + 
  geom_hline(yintercept= -log10(0.05), linetype="dashed")
ggsave('figures/slim_pcangsdoutlier.png', plot = p, width = 8, height = 16, dpi = 300)
...
# plot proprtion pfdr<0.05 vs. s and ne (group by f)
