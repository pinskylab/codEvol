# Aborted attempt to re-write wfs_nullmodel_function.r to re-run low p-value loci
...
# arguments (order must be exactly this, argument processor is not sophisticated)
# 	myalcnt1
#	myalcnt2
#	pop
#	myyr1
#	myyr2
#	rerunlow
#	maxcores (optional, but required if myalcnt3 is used)
#	myalcnt3 (optional)
...
if (length(args)<6) {
  stop("Have to specify at least myalcnt1, myalcnt2, pop, myyr1, myyr2, rerunlow", call.=FALSE)
...
if(rerunlow==1){
	print('Trimming to loci with p<=(3+1)/(n+1)')
	if(pop=='Lof'){
	...
	targ <- merge(targ, pvals[,.(CHROM, POS, n, p)], by=c('CHROM', 'POS')) # merge in p-values
	print(nrow(targ))		
	targ <- targ[p<=(3+1)/(n+1),]
	print(nrow(targ))
}
...
if(pop %in% c('Lof', 'Pow')){
	if(myyr2 != '1114'){
		...
		if(rerunlow==1){
			ffnm <- list.files(path='analysis/temp', pattern=paste('wfs_simsnull_ff', paste(myalcnt1, myalcnt2, sep=','), '_[[:digit:]]+\\.ffData', sep=''), full.names=TRUE) # the existing simulations
			ffnm <- gsub('.ffData', '', ffnm)
		}
	}
	if(myyr2 == '1114'){
		...
		if(rerunlow==1){
			ffnm <- list.files(path='analysis/temp', pattern=paste('wfs_simsnull_ff', paste(myalcnt1, myalcnt2, myalcnt3, sep=','), '_[[:digit:]]+\\.ffData', sep=''), full.names=TRUE) # the existing simulations
			ffnm <- gsub('.ffData', '', ffnm)
		}
...
if(pop %in% c('Can', 'PowCan')){
	...
	if(rerunlow==1){
		ffnm <- list.files(path='analysis/temp', pattern=paste('wfs_simsnullCAN_ff', paste(myalcnt1, myalcnt2, sep=','), '_[[:digit:]]+\\.ffData', sep=''), full.names=TRUE) # the existing simulations
		ffnm <- gsub('.ffData', '', ffnm)
	}
}
...
nullmodtest <- function(locusnum, thistarg, thisout.ff, tol=1/100){
	...
	return(c(locusnum=locusnum, p=p, r=sum(extinds), n=sum(stinds)))
}
...
# only do this if starting from scratch. not useful if rerunning loci with low p-values
...
if(sampleparts.n > 0){
	...
	for(partnum in parts){
		...
		res <- data.frame(locusnum=numeric(0), p=numeric(0), r=numeric(0),n=numeric(0)) # vector of number of simulations more extreme than observations and count of simulations with same starting sample freq. needed mostly if length(ffnm)>1 (if rerunlow==1)

		# loop through the files of simulations
		for(j in 1:length(ffnm)){
			# load the appropriate file of abc simulations
				# null model
			ffload(ffnm[j], overwrite=TRUE, rootpath=getOption('fftempdir')) # takes 30 sec or so. loads thisout.ff
			...
			if(myyr2 != '1114'){
				results <- foreach(i=thisdat, .combine=rbind, .packages=c('ff', 'doParallel', 'data.table')) %dopar% {
					nullmodtest(locusnum=i[,locusnum], thistarg=i[,.(f1samp, f2samp)], thisout.ff=thisout.ff) # run null model test. returns vector of locusnum, p-value, count of sims more extreme, and count of simulations with same starting sample freq
				}
			}
			if(myyr2 == '1114'){ # 3 time points
				results <- foreach(i=thisdat, .combine=rbind, .packages=c('ff', 'doParallel', 'data.table')) %dopar% {
					nullmodtest3(locusnum=i[,locusnum], thistarg=i[,.(f1samp, f2samp, f3samp)], thisout.ff=thisout.ff) # run null model test. returns vector of locusnum, p-value, count of sims more extreme, and count of simulations with same starting sample freq
				}
			}
			...
			# might be better to leave this, since will be needed for next partnum? but not sure how to remove at the end if I do that
			delete(thisout.ff)
			rm(thisout.ff)

		}
		
		# combine all res into one line per locus
		res2 <- res[,.(r=sum(r), n=sum(n)), by=locusnum]
		res2[, p:= (r+1)/(n=1)]
		...
		write.csv(res2, file=gzfile(outfile), row.names=FALSE) # write directly to gzipped file. has locusnum, r, n (number of simulations), and p
