...
dat14 <- merge(dat14, outl[,.(CHROM, POS, kmer25, dpLofFlag, outlier=p.Lof.adj3<0.3)], by.x=c('CHR', 'POS1'), by.y=c('CHROM', 'POS'), all.x=TRUE) # on POS1
dat14 <- merge(dat14, outl[,.(CHROM, POS, kmer25, dpLofFlag, outlier=p.Lof.adj3<0.3)], by.x=c('CHR', 'POS2'), by.y=c('CHROM', 'POS'), all.x=TRUE) # on POS2

dat11 <- merge(dat11, outl[,.(CHROM, POS, kmer25, dpLofFlag, outlier=p.Lof.adj3<0.3)], by.x=c('CHR', 'POS1'), by.y=c('CHROM', 'POS'), all.x=TRUE)
dat11 <- merge(dat11, outl[,.(CHROM, POS, kmer25, dpLofFlag, outlier=p.Lof.adj3<0.3)], by.x=c('CHR', 'POS2'), by.y=c('CHROM', 'POS'), all.x=TRUE)
	
dat07 <- merge(dat07, outl[,.(CHROM, POS, kmer25, dpLofFlag, outlier=p.Lof.adj3<0.3)], by.x=c('CHR', 'POS1'), by.y=c('CHROM', 'POS'), all.x=TRUE)
dat07 <- merge(dat07, outl[,.(CHROM, POS, kmer25, dpLofFlag, outlier=p.Lof.adj3<0.3)], by.x=c('CHR', 'POS2'), by.y=c('CHROM', 'POS'), all.x=TRUE)

dat40 <- merge(dat40, outl[,.(CHROM, POS, kmer25, dpCanFlag, outlier=p.Can.adj3<0.3)], by.x=c('CHR', 'POS1'), by.y=c('CHROM', 'POS'), all.x=TRUE)
dat40 <- merge(dat40, outl[,.(CHROM, POS, kmer25, dpCanFlag, outlier=p.Can.adj3<0.3)], by.x=c('CHR', 'POS2'), by.y=c('CHROM', 'POS'), all.x=TRUE)

datMod <- merge(datMod, outl[,.(CHROM, POS, kmer25, dpCanFlag, outlier=p.Can.adj3<0.3)], by.x=c('CHR', 'POS1'), by.y=c('CHROM', 'POS'), all.x=TRUE)
datMod <- merge(datMod, outl[,.(CHROM, POS, kmer25, dpCanFlag, outlier=p.Can.adj3<0.3)], by.x=c('CHR', 'POS2'), by.y=c('CHROM', 'POS'), all.x=TRUE)
...
exclude <- dat14[outlier==TRUE, unique(c(paste(CHR, POS1), paste(CHR, POS2)))] # list of loci that are outliers or are <5000 bp from outliers
...
dat14 <- dat14[outlier==TRUE | notoutlier==TRUE,]
dat11 <- dat11[outlier==TRUE | notoutlier==TRUE,]
dat07 <- dat07[outlier==TRUE | notoutlier==TRUE,]
dat40 <- dat40[outlier==TRUE | notoutlier==TRUE,]
datMod <- datMod[outlier==TRUE | notoutlier==TRUE,]
...
# write out
write.csv(bins, file='analysis/ld_outliers.csv')
...
bins <- fread('analysis/ld_outliers.csv')
...
# pdf(width=6, height=3, file='figures/ld_decay_outliers.pdf')
...
bins[pop=='LOF_S_11' & outlier==TRUE,lines(distclass, r2ave, type='l', cex=cex, col=cols[2], lwd=lwd)]
bins[pop=='LOF_07' & outlier==TRUE,lines(distclass, r2ave, type='l', cex=cex, col=cols[3], lwd=lwd)]

bins[pop=='LOF_S_14' & outlier==FALSE,lines(distclass, r2ave, type='l', cex=cex, col=cols[1], lty=3, lwd=lwd)]
bins[pop=='LOF_S_11' & outlier==FALSE,lines(distclass, r2ave, type='l', cex=cex, col=cols[2], lty=3, lwd=lwd)]
bins[pop=='LOF_07' & outlier==FALSE,lines(distclass, r2ave, type='l', cex=cex, col=cols[3], lty=3, lwd=lwd)]
...
bins[pop=='LOF_S_11' & outlier==TRUE,lines(c(distclass, distclass), c(r2ave-1.96*r2se, r2ave+1.96*r2se), col=cols[2]), by=distclass]
bins[pop=='LOF_07' & outlier==TRUE,lines(c(distclass, distclass), c(r2ave-1.96*r2se, r2ave+1.96*r2se), col=cols[3]), by=distclass]

bins[pop=='LOF_S_14' & outlier==FALSE,lines(c(distclass, distclass), c(r2ave-1.96*r2se, r2ave+1.96*r2se), col=cols[1]), by=distclass]
bins[pop=='LOF_S_11' & outlier==FALSE,lines(c(distclass, distclass), c(r2ave-1.96*r2se, r2ave+1.96*r2se), col=cols[2]), by=distclass]
bins[pop=='LOF_07' & outlier==FALSE,lines(c(distclass, distclass), c(r2ave-1.96*r2se, r2ave+1.96*r2se), col=cols[3]), by=distclass]
...
bins[pop=='CAN40' & outlier==TRUE,lines(distclass, r2ave, type='l', cex=cex, col=cols[5], lwd=lwd)]

bins[pop=='CANMod' & outlier==FALSE,lines(distclass, r2ave, type='l', cex=cex, col=cols[4], lty=3, lwd=lwd)]
bins[pop=='CAN40' & outlier==FALSE,lines(distclass, r2ave, type='l', cex=cex, col=cols[5], lty=3, lwd=lwd)]

bins[pop=='CANMod' & outlier==TRUE,lines(c(distclass, distclass), c(r2ave-1.96*r2se, r2ave+1.96*r2se), col=cols[4]), by=distclass]
bins[pop=='CAN40' & outlier==TRUE,lines(c(distclass, distclass), c(r2ave-1.96*r2se, r2ave+1.96*r2se), col=cols[5]), by=distclass]
bins[pop=='CANMod' & outlier==FALSE,lines(c(distclass, distclass), c(r2ave-1.96*r2se, r2ave+1.96*r2se), col=cols[4]), by=distclass]
bins[pop=='CAN40' & outlier==FALSE,lines(c(distclass, distclass), c(r2ave-1.96*r2se, r2ave+1.96*r2se), col=cols[5]), by=distclass]
