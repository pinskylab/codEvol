...
# to allow scaling of target stats and simulation stats
	...
	meansds[,f2samp.mean:=numeric(nrow(meansds))]
	meansds[,f2samp.sd:=numeric(nrow(meansds))]
	...
	for(i in 1:nrow(meansds)){ # this takes 22%+ of a cod node RAM... why? perhaps because mean and sd on vectors of length 10M
		...
		meansds[i,f2samp.mean:=mean(out['f2samp',,j], na.rm=TRUE)]
		meansds[i,f2samp.sd:=sd(out['f2samp',,j], na.rm=TRUE)]
	...
	meansds[f2samp.sd==0, f2samp.sd:=1] # set sd = 1 for cases were sd=0, to prevent Inf
...
# set up target statistics
	...
	targ <- cbind(1:nrow(sampsize), sampsize[,.(V1, V2)], alcnts[,V1]/sampsize[,V1], alcnts[,V2]/sampsize[,V2], obs[,.(Fsd, Fsi)]) # alcnt1, alcnt2, f1samp, fsd, fsi
	setnames(targ, 1:7, c('locusnum', 'alcnt1', 'alcnt2', 'f1samp', 'f2samp', 'fsd', 'fsi'))
	...
	targ[,f2samp.n:=(f2samp-f2samp.mean)/f2samp.sd]
	targ[,fsd.n:=(fsd-fsd.mean)/fsd.sd]
	targ[,fsi.n:=(fsi-fsi.mean)/fsi.sd]

	# re-order rows and columns
	setkey(targ, locusnum)
	setcolorder(targ, c('locusnum', 'alcnt1', 'alcnt2', 'f1samp.n', 'f2samp.n', 'fsd.n', 'fsi.n', 'f1samp', 'f2samp', 'fsd', 'fsi', 'f1samp.mean', 'f2samp.mean', 'fsd.mean', 'fsi.mean', 'f1samp.sd', 'f2samp.sd', 'fsd.sd', 'fsi.sd')) # once meansds has f2samp data
#	setcolorder(targ, c('locusnum', 'alcnt1', 'alcnt2', 'fsd.n', 'fsi.n', 'f1samp', 'f2samp', 'fsd', 'fsi', 'f1samp.mean', 'fsd.mean', 'fsi.mean', 'f1samp.sd', 'fsd.sd', 'fsi.sd'))
...
# also scale simulation statistics to mean 0 sd 1
	...
	checkscl[,f2samp.mean:=numeric(nrow(checkscl))]
	checkscl[,f2samp.sd:=numeric(nrow(checkscl))]
	checkscl[,f2samp.na:=numeric(nrow(checkscl))]
...
#		thisout['f2samp',] <- (thisout['f2samp',]-meansds[i,f2samp.mean])/meansds[i,f2samp.sd]
		...
		checkscl[i,f2samp.mean := mean2(thisout['f2samp',])]
		checkscl[i,f2samp.sd := sd(thisout['f2samp',], na.rm=TRUE)]
		...
		checkscl[i,f2samp.na := sum(is.na(thisout['f2samp',]))]
...
#	checkscl[f2samp.mean > 1e-12 | f2samp.mean < -1e-12 | f2samp.sd > 1.0001 | f2samp.sd < 0.9999,]
